...
import { FINGERPRINT_SHORT, Scramble } from './textfx';
...
interface RegistryPanelProps {
  ...
  showTx?: boolean;
  /** When set, the fingerprint resolves from cycling hex instead of sitting static. */
  resolveFingerprint?: boolean;
}
...
export function RegistryPanel({ width = '43vw', status, showTx = false, resolveFingerprint }: RegistryPanelProps) {
    ...
    <Panel width={width}>
      ...
      <div style={{ padding: '1.3vw 1.6vw', display: 'grid', gridTemplateColumns: '1.5fr 0.7fr 1fr', gap: '1vw' }}>
        <div>
          ...
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '1.32vw', color: INK }}>
            {resolveFingerprint === undefined ? FINGERPRINT_SHORT : <Scramble to={FINGERPRINT_SHORT} from={'·'.repeat(FINGERPRINT_SHORT.length)} active={resolveFingerprint} duration={900} />}
          </div>
        </div>
        <div>
      ...
      <div className="flex items-center justify-between" style={{ padding: '1vw 1.6vw 1.2vw', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <motion.div
          ...
          style={{ gap: '0.5vw', fontFamily: 'var(--font-mono)', fontSize: '1.05vw', whiteSpace: 'nowrap', color: showTx ? MUTED : DIM }}
          initial={{ opacity: 0 }}
