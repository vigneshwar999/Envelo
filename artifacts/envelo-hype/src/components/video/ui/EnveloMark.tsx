...
interface EnveloMarkProps {
  size: string;
  /** Seconds after mount (or after `shineKey` changes) when the light sweep should cross the mark. */
  shineAt?: number;
  /** Changing this replays the sweep; lets a persistent mark shine again on later scenes. */
  shineKey?: string | number;
  className?: string;
}

export function EnveloMark({ size, shineAt, shineKey = 0, className }: EnveloMarkProps) {
